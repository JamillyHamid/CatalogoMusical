...
const updateDiscoById = async (discoId, titulo, anoLancamento, capa, genero, nome, duracao, audio) => {
    const transaction = await sequelize.transaction();
    try {
        console.log({
            discoId,
        ...
        await sequelize.query(
            ...
            {
                replacements: { titulo, anoLancamento, capa, discoId },
                transaction,
            }
        );
        if (genero) {
            await sequelize.query(
                `UPDATE generomusical
                 ...
                 WHERE discoFk = :discoId`,
                {
                    replacements: { genero, discoId },
                    transaction,
                }
            );
        }
        if (nome || duracao || audio) {
            await sequelize.query(
                `UPDATE faixa
                 SET nome = :nome,
                     duracao = :duracao,
                 ...
                 WHERE discoFk = :discoId`,
                {
                    replacements: { nome, duracao, audio, discoId },
                    transaction,
                }
            );
        }
