...
export const showDiscos = async (req, res) => {
    try {
        ...
        const artistas = await artistaModel.findAllArtista(); // Novo método
...
export const findDiscoById = async (req, res) => {
    try {
        ...
        console.log('ESSA POSSRA', discos);
        if(!discos) {
...
export const buscarDisco = async (req, res) => {
    ...
    try {
        // Busca discos
        const discos = await discosModel.findDisco({ titulo, artista, genero, generoMusical });
        for (const disco of discos) {
            disco.generos = await discosModel.findGenerosByDiscoId(disco.discoId);
            disco.faixas = await discosModel.findFaixasByDiscoId(disco.discoId);
        }
        ...
        // Busca artistas
        const artistas = await artistaModel.findArtista({ titulo, artista, genero, generoMusical });

        console.log(discos, artistas);

        // Renderiza a página com os objetos
        res.render('index', { 
            discos, 
            artistas,
            filtros: req.query,
